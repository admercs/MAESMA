// MAESMA: Modular Agentic Earth System Modeling Arena
// A Typst paper describing the architecture and design

#set document(
  title: "MAESMA: Autonomous Agentic AI for Earth System Model Discovery via Knowledgebase-Centric Neural Inference",
...
#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 [
      ...
      MAESMA: Autonomous Agentic Earth System Modeling
      #h(1fr)
...
#block(
  width: 100%,
  inset: (x: 0.3in, y: 0.4em),
  stroke: none,
)[
  #set par(first-line-indent: 0em)
  #text(weight: "bold")[Abstract.]
  Earth system models (ESMs) are among the most complex scientific software ever constructed, yet their development remains a largely manual endeavor: researchers hand-select process representations, tune parameters, and evaluate skill through labor-intensive campaigns. We present MAESMA --- the Modular Agentic Earth System Modeling Arena --- a multi-agent system for _inferential process discovery, combination, simulation, and evolution_. An autonomous swarm of 25 AI agents discovers, assembles, benchmarks, selects, and _invents_ Earth system model configurations without human intervention. At the center of the architecture lies a _Process Knowledgebase_: a single versioned store of all process models, machine-readable manifests, ontological metadata, and empirical skill records. A _neural inference engine_ --- a graph transformer trained on the knowledgebase --- reasons over process embeddings, simulation error fields, regime context, and compute budgets to propose process selections, replacing hand-crafted heuristics with learned reasoning. Errors and uncertainties from simulations propagate back as the primary signal driving both selection and discovery: every model--observation mismatch becomes a query against the knowledgebase. A _process discovery pipeline_ learns new representations from observational data via Fourier Neural Operators (FNO), Physics-Informed Neural Operators (PINO), DeepONet, MeshGraphNet, symbolic regression, and physics--ML hybrids --- integrating NVIDIA Modulus principles for PDE-constrained training on GPU --- validates them, and deposits them into the knowledgebase. Discovered representations then _evolve_: the system recombines, mutates, and selects process operators via evolutionary search over the knowledgebase graph, treating the Pareto front as a fitness landscape. Drawing on artificial life (ALife) principles inspired by Conway Research's _automaton_ framework, MAESMA treats every process representation as a _sovereign living entity_ that must earn its existence through predictive skill: survival tiers (normal, low-compute, critical, archived) gate compute allocation, constitutional invariants (conservation, provenance, earned existence) propagate to all offspring, a heartbeat daemon continuously evaluates fitness, and stagnant processes die while high-fitness processes self-replicate --- producing a phylogenetic tree of process lineages. Foundation weather models from NVIDIA Earth-2 (FourCastNet, GraphCast, GenCast) provide GPU-accelerated atmospheric state evolution at 1000$times$ the speed of dynamical models, while autonomous observation intelligence inspired by ESA PhiSat-2 deploys edge-AI classifiers to satellites, drones, and sensor networks to steer data acquisition toward maximal information gain. The system covers all DOE EESM program areas (ESMD, RGMA, MSD), 13 process families spanning atmosphere through geomorphology, geoengineering feedback control with 14 intervention types, and planetary defense modeling calibrated against the full paleontological record of mass extinctions. GPU acceleration is a first-class architectural concern: state arrays remain device-resident, process operators execute as compute shaders, foundation model inference and physics-informed training run end-to-end on GPU. Inter-institutional collaboration is achieved via the A2A federation protocol, enabling federated model assembly and skill sharing without centralizing proprietary data. MAESMA is implemented in Rust with a Next.js monitoring dashboard.
]
...
Earth system models couple atmosphere, ocean, cryosphere, biosphere, and human systems to simulate planetary dynamics across spatial scales from meters to the globe and temporal scales from seconds to millennia. Despite decades of development, the construction of these models remains fundamentally manual. Modelers choose process representations based on expertise and convention, tune parameters through trial-and-error campaigns, and evaluate skill against limited observation suites. The combinatorial space of possible configurations --- which representations to include, at what resolution, in what coupling arrangement --- is vast, yet only a tiny fraction is ever explored.

Three structural limitations constrain progress. First, _process selection is heuristic_: the decision to use, say, a Rothermel fire spread model versus a wind-aware plume model is made by human judgment rather than systematic evaluation. Second, _process discovery is decoupled from modeling_: when simulations exhibit persistent biases, the feedback loop from error to new process representation is slow, informal, and largely manual. Third, _knowledge is fragmented_: process implementations, their metadata, performance records, and ontological relationships are scattered across code repositories, papers, and institutional memory rather than unified in a single queryable store.

MAESMA addresses all three limitations through a knowledgebase-centric architecture in which an autonomous agent swarm reasons over a central Process Knowledgebase via a neural inference engine. The system runs indefinitely, discovering data, assembling model configurations, benchmarking against observations, selecting optimal structures, discovering new process representations from residual analysis, and depositing them back into the knowledgebase --- all without human gating. Humans monitor via a live dashboard and may optionally adjust objectives or budgets, but the workflow proceeds autonomously.

The contributions of this paper are:
#set par(first-line-indent: 0em)
+ A _knowledgebase-centric architecture_ that unifies process code, manifests, ontological relations, skill records, and provenance in a single versioned store.
+ A _salient-dynamics-first selection principle_ in which agents prioritize processes with the greatest effect on the time evolution of system states, adding lower-impact detail incrementally as budget and accuracy targets demand.
+ A _neural inference engine_ (graph transformer) that reasons over the knowledgebase to propose process selections driven by simulation errors and uncertainties.
+ An _autonomous optimization loop_ with four concurrent cycles (fitness optimization, data discovery, regime discovery, process discovery) that runs indefinitely.
+ A _process discovery and evolution pipeline_ that detects persistent biases, learns new representations via NVIDIA Modulus-trained neural operators and symbolic regression, validates them, deposits them into the knowledgebase, and _evolves_ them via recombination, mutation, and Pareto-optimal selection over the process graph.
+ An _artificial life (ALife) process lifecycle_ inspired by Conway Research's automaton: every process representation is a sovereign living entity with survival tiers (normal, low-compute, critical, archived), constitutional invariants (conservation, provenance, earned existence), self-replication for high-fitness candidates, a heartbeat daemon for continuous survival evaluation, and phylogenetic lineage tracking --- treating autonomous model construction and selection as an evolutionary optimization process.
+ _Foundation weather model integration_ (NVIDIA Earth-2): pre-trained AI weather models (FourCastNet, GraphCast, GenCast) as GPU-accelerated Atmosphere R0/R1 rungs providing 1000$times$ faster atmospheric state evolution.
+ _Autonomous observation intelligence_ (PhiSat-2 principles): edge-AI classifiers deployed to satellites, drones, and sensor networks for on-board data filtering and active observation tasking driven by model uncertainty.
+ Coverage of 13 process families, 25 specialized agents, geoengineering feedback control with 14 intervention types, and planetary defense calibrated against 13 mass extinction events.
+ An _A2A federation protocol_ for inter-institutional agent collaboration and skill sharing.
...
MAESMA organizes its agent swarm into three concurrent control layers:
...
The strategic layer sets objectives and drives the Pareto-optimal selection of model structures. The tactical layer assembles, benchmarks, and discovers. The operational layer compiles, schedules, and monitors execution. The strategic loop never terminates.
...
GPU acceleration is a first-class architectural concern in MAESMA, not an afterthought. Three pillars support GPU-native execution across the full pipeline --- process stepping, neural inference, and process discovery:

*GPU-resident state management.* Simulation state arrays are allocated on GPU memory and remain device-resident across timesteps. Process operators read from and write to GPU buffers directly via `wgpu` compute shaders (portable across Vulkan, Metal, DX12) or CUDA kernels via `cudarc` bindings. CPU↔GPU transfers occur only at checkpoint boundaries and observation ingestion, minimizing PCIe bottleneck.

*Batched ensemble execution.* The runtime schedules multiple ensemble members, regions, and rung-swap experiments as concurrent GPU workgroups. A single kernel launch can advance hundreds of independent grid cells or cohort populations in parallel. This is critical for the autonomous optimizer, which must evaluate many candidate configurations per cycle to maintain Pareto front coverage.

*Neural operator inference on device.* The graph transformer inference engine and all neural operator surrogates (FNO, DeepONet, MeshGraphNet) execute their forward passes on GPU, avoiding round-trip serialization to a Python subprocess. Inference results (candidate rankings, skill predictions, regime classifications) are written directly to GPU-side buffers consumed by the scheduler.

*Physics-informed training loop (NVIDIA Modulus integration).* The process discovery pipeline integrates principles from NVIDIA Modulus for physics-informed neural operator training. When learning new representations from persistent residuals, the training loop enforces PDE residual constraints, boundary conditions, and conservation laws as differentiable loss terms computed entirely on GPU. Geometry-adaptive Fourier Neural Operators (Geo-FNO) handle irregular domains (coastlines, topography, watershed boundaries) without remeshing. The Modulus model zoo --- including FNO, Physics-Informed Neural Operator (PINO), DeepONet, and MeshGraphNet --- provides pre-validated architectures that the discovery pipeline instantiates, trains, and validates before depositing into the knowledgebase.

*Multi-GPU scaling.* For high-resolution coupled simulations, domain decomposition distributes spatial tiles across multiple GPUs via NCCL collective operations. Halo exchanges between tiles use GPU-direct RDMA where available, falling back to staged CPU copies. The federation protocol extends this to multi-node, multi-institution GPU clusters.
...
*Diagnostic and scoring pipelines.* Earth-2 diagnostic workflows compute derived quantities (accumulated cyclone energy, atmospheric rivers, heat wave indices, fire weather indices) from foundation model outputs on GPU, feeding directly into MAESMA's benchmarking agent for skill scoring. Perturbation methods (Bred Vectors, Gaussian, Spherical Harmonics) generate calibrated uncertainty estimates that propagate through the coupled system.

All Earth-2 models are accessed through a unified `FoundationModelRunner` trait that implements `ProcessRunner`, exposing the same interface as physics-based process operators. Model weights are stored in the knowledgebase as ONNX artifacts, enabling versioned, content-addressed weight management. The Data Scout Agent monitors the Earth-2 model zoo for new releases and automatically registers them as candidate representations.

== Autonomous Observation Intelligence (PhiSat-2)

MAESMA extends the observation pipeline from passive data ingestion to _autonomous observation intelligence_, integrating principles from ESA's PhiSat-2 mission --- the first satellite to deploy on-board AI inference for real-time Earth observation filtering and classification.

The PhiSat-2 paradigm introduces three capabilities that transform MAESMA's data-driven workflow:

*Edge-AI observation filtering.* In the PhiSat-2 architecture, an on-board neural network (deployed on a low-power AI accelerator) classifies raw imagery in real time, discarding cloud-contaminated frames and prioritizing scientifically valuable scenes before downlink. MAESMA generalizes this principle: the Autonomous Observation Agent deploys lightweight classifier models to edge compute nodes (satellite ground segments, field sensor networks, drone platforms) that filter and prioritize incoming data streams. Only data that reduces epistemic uncertainty in the knowledgebase --- as estimated by the neural inference engine --- is ingested, transmitted, and processed. This reduces data volume by 1--2 orders of magnitude while increasing information density.

*Active observation tasking.* Beyond passive filtering, the Autonomous Observation Agent issues _observation requests_ to programmable platforms. When the process discovery pipeline identifies a persistent structured bias in a particular region--regime combination, the agent computes a value-of-information score for each available observation modality (SAR, optical multispectral, thermal, lidar, in-situ) and submits tasking requests to satellite operators, UAV fleets, or sensor networks. The observation becomes part of the autonomous optimization loop: bias detected → observation requested → data acquired → knowledgebase updated → process re-scored.

*On-board inference for process monitoring.* PhiSat-2 demonstrates that process-relevant feature extraction (fire detection, flood mapping, vegetation stress, land cover change) can occur at the point of acquisition. MAESMA designates these on-board classifiers as knowledgebase entries of type `edge-inference`, with manifests specifying their input contracts (spectral bands, spatial resolution), output products (fire probability maps, cloud masks, change indices), latency characteristics, and skill records validated against ground truth. The process discovery pipeline can deploy newly discovered classifiers to edge nodes, closing the loop from knowledgebase to orbit.

The observation intelligence layer is coordinated by the Autonomous Observation Agent, which maintains an _observation value map_ --- a spatiotemporal field estimating the marginal information gain of each potential observation. This map is derived from the neural inference engine's uncertainty estimates, the knowledgebase's coverage gaps, and the active learning agent's experiment queue. The result is a system that does not merely consume observations but actively steers the global observation network to maximize knowledge gain per bit transmitted.
...
Each knowledgebase entry bundles five layers:
...
The knowledgebase supports five core operations:
...
The knowledgebase is indexed by a unified ontology spanning five interconnected domains:
...
The core execution cycle runs indefinitely with six steps:
...
The Autonomous Optimizer maintains a Pareto frontier over skill versus cost. Optimization operates at three nested scopes:
...
#figure(
  ...
  caption: [Three nested optimization scopes.],
) <tab:scopes>
...
Three classes of learned representations are supported:
...
MAESMA organizes process representations into 13 families, each with a _representation ladder_ of increasing fidelity (R0--R3):
...
A Next.js dashboard provides real-time monitoring via WebSocket subscriptions:
...
The Rust codebase is organized as a Cargo workspace of 10 crates, each with a focused responsibility:
...
MAESMA is governed by 23 design principles. We highlight the most distinctive:
...
We have presented MAESMA, a knowledgebase-centric multi-agent system for _inferential process discovery, combination, simulation, and evolution_ in Earth system modeling. By placing a central Process Knowledgebase at the architectural center and equipping a 25-agent swarm with a neural inference engine that reasons over it, MAESMA transforms the traditionally manual process of ESM development into an autonomous, indefinitely running cycle of data discovery, model assembly, benchmarking, selection, process discovery, process evolution, and knowledgebase update. Drawing on artificial life (ALife) principles from Conway Research's automaton framework, MAESMA treats every process representation as a sovereign living entity: survival tiers gate compute allocation based on skill-to-cost ratio, constitutional invariants (conservation, provenance, earned existence) are immutable and propagate to all offspring, a heartbeat daemon continuously evaluates fitness and triggers tier transitions, and high-fitness processes self-replicate while stagnant processes die --- producing a phylogenetic tree of process lineages that records the complete evolutionary history of the modeling system. Foundation weather models from NVIDIA Earth-2 (FourCastNet, GraphCast, GenCast) serve as GPU-accelerated Atmosphere R0/R1 rungs, providing 1000$times$ faster atmospheric state evolution and freeing GPU budget for the process families with the greatest impact on system dynamics. Autonomous observation intelligence inspired by ESA PhiSat-2 deploys edge-AI classifiers from orbit to field sensors, steering data acquisition toward maximal information gain. The process discovery pipeline --- powered by NVIDIA Modulus-trained neural operators (FNO, PINO, DeepONet, MeshGraphNet) --- not only discovers new representations from persistent residuals but _evolves_ them via crossover, mutation, and Pareto-optimal selection, treating the knowledgebase as a genome and the skill--cost frontier as a fitness landscape. The system covers 13 process families across all DOE EESM program areas, supports geoengineering feedback control with 14 intervention types, models all known mass extinction drivers calibrated against 4.5 billion years of Earth history, and enables inter-institutional collaboration via A2A federation. GPU acceleration is a first-class architectural concern end-to-end: state arrays remain device-resident, process operators execute as wgpu compute shaders with CUDA fast-paths, foundation model inference runs on GPU, and Modulus physics-informed training loops execute entirely on device. A reference implementation realized as a 10-crate Rust workspace with a Next.js monitoring dashboard demonstrates that the architecture translates directly to high-performance, type-safe code. The initial Process Knowledgebase is seeded with 1185 entries drawn from 50 reference models and platforms --- spanning ocean biogeochemistry (MARBL, BFM), ice sheet and glacier dynamics (PISM, OGGM), permafrost thermodynamics (CryoGrid, ATS), coastal morphodynamics (XBeach), agricultural systems (APSIM, DSSAT), urban energy balance (SURFEX/TEB), marine food webs (EwE), aerosol microphysics (CARMA), GPU-native models (SORTIE-NG, DeepLand), foundation model platforms (Earth-2), autonomous observation systems (PhiSat-2), and one benchmarking dataset (FireBench) --- each augmented with machine-readable manifests upon which the neural inference engine begins its first reasoning cycle. Every simulation error becomes a query against the knowledgebase; every discovered process becomes a living automaton under evolutionary selection pressure --- and the system's knowledge compounds over time.
...
#align(center)[
  ...
  #v(0.4em)
  #text(size: 11pt, fill: luma(100))[Initial Process Knowledgebase Catalog]
  #v(0.6em)
]
