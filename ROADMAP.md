...
- [ ] Grid↔grid conservative remapping (aggregation + disaggregation)
- [ ] Grid↔network transfer, state projection (individual↔cohort), temporal coupling operators
- [ ] Information-loss tracking for every projection/aggregation
...
- [ ] Training loop: skill score deltas from accepted proposals as reward signal; incremental retraining as KB grows
- [ ] Uncertainty-aware outputs: calibrated confidence per proposal; low-confidence routes to Active Learning
- [ ] Integration with Knowledgebase Retrieval Agent and Autonomous Optimizer Agent
...
- [ ] **H1**: Multi-layer Richards + infiltration (30–300 m, 5–60 min)
- [ ] **Routing**: Kinematic wave → diffusive wave
...
- [ ] Validation: PBDB, TimeTree, TRY, BIEN, LTER, Quaternary pollen records
...
- [ ] Register standard metrics + scoring protocols + default fitness functions
...
- [ ] Expert-prior skill models per Representation: accuracy class per regime, known failure modes
- [x] Cost models: FLOPS/cell/timestep, memory, GPU/CPU preference, scaling behavior
- [x] `compatible_with`/`incompatible_with` edges
- [ ] Mark all as updatable by the knowledgebase learning loop (distinguish expert priors from empirical posteriors)
...
- [ ] Parse user objectives → observable requirements + error bands + priority tiers

### 5.2 Knowledgebase Retrieval Agent
- [ ] Query Process Knowledgebase via neural inference engine for candidates given errors, scale/regime/budget/data
- [ ] Rank candidates by inferred error-reduction potential, cost/skill tradeoff

### 5.3 Model Assembly Agent
- [ ] Build candidate SAPG from neural inference engine proposals; pick rungs, declare embeddings, propose coupling cadence

### 5.4 Closure & Consistency Agent
- [ ] Automated validation: missing variables, double-counted physics, unit mismatches, conservation violations, CFL

### 5.5 Provenance & Audit Agent
- [ ] Generate "why this model" reports: selections, rejections, sensitivity hotspots

### 5.6 Benchmarking Agent
- [ ] Run/schedule simulations; extract outputs at observation locations; compute multi-metric skill; write to Store; compare against existing records

### 5.7 Model Selection Agent
- [ ] Neural inference + posterior $p(M_k | \mathbf{y}) \propto p(\mathbf{y} | M_k)\,p(M_k)$; marginal likelihoods; online Bayesian updating; BMA ensemble weights
- [ ] Inference engine proposes candidates; Bayesian framework validates and updates posteriors

### 5.8 Active Learning Agent
- [ ] Identify high-uncertainty configurations, under-observed regimes, sensitivity frontiers
- [ ] Propose transferability tests (cross-region validation)
- [ ] Output prioritized experiment queue ranked by expected information gain

### 5.9 Skill Librarian Agent
- [ ] Manage Store lifecycle (writes, queries, versioning, GC); aggregate across dimensions; detect skill drift; generate trend reports

### 5.10 Autonomous Optimizer Agent
- [ ] Fitness function: $F(r,g,\ell) = \sum w_m S_m(r,g,\ell) - \lambda C(r)$
- [ ] Per-region, per-regime loop: neural inference queries KB → compute fitness → Pareto frontier → swap if dominant → delegate to Active Learning if uncertain
- [ ] Convergence detection, budget-aware mode, regime-aware mode; full provenance logging

### 5.11 Data Scout Agent
- [ ] Gap analysis: identify observables/regions/periods with unreliable skill estimates; score by information gain
- [ ] Catalog search: STAC, NASA CMR, Copernicus, CKAN with spatial/temporal/keyword filters
- [ ] Relevance scoring: overlap, resolution compatibility, uncertainty adequacy, variable mapping
- [ ] Novelty scoring: new coverage, regime coverage, independence from existing products
- [ ] Ingest pipeline: auto-generate Product manifest + TransformRecipe → preprocess → deposit into KB → re-score
- [ ] Governance: allowlisted sources, license check, rate limiting, optional first-ingest approval

### 5.12 A2A Gateway Agent
- [ ] Agent Card schema + publication at `/.well-known/agent.json`
- [ ] Peer discovery, capability caching, health status
- [ ] Task lifecycle: `submitted → working → input-required → completed / failed / canceled`
- [ ] Artifact exchange (IR fragments, skill records, manifests)
- [ ] Auth (bearer tokens, mutual TLS), trust scoring, rate limiting, circuit breakers

### 5.13 MSD Coupling Agent
- [ ] Bidirectional coupling: natural → human (climate impacts) and human → natural (emissions, land use, water extraction)
- [ ] Coupling frequency negotiation; commodity/resource accounting
- [ ] Cascading impact propagation: drought → water → energy → economic loss
- [ ] MSD scenario parameter space (SSP × RCP × technology × policy)

### 5.14 Scenario Discovery Agent
- [ ] AI-driven exploration: Latin hypercube / Sobol sampling, cluster outcomes, identify tipping points
- [ ] Sensitivity analysis, scenario trees, Dynamical Digital Testbeds
- [ ] Extreme event compounding analysis (drought + heatwave + infrastructure failure)

### 5.15 EESM Diagnostics Agent
- [ ] ILAMB, IOMB, E3SM Diagnostics, PMP wrappers
- [ ] Multi-component evaluation campaigns
- [ ] RGMA thrust diagnostics: cloud, BGC, high-latitude, variability, extremes, water cycle
- [ ] Standardized reports (HTML/PDF) with CMIP baselines

### 5.16 Process Discovery Agent
- [ ] Structured residual analysis: systematic bias detection, attribution (variables, regions, seasons, regimes), persistence test, severity scoring
- [ ] Hypothesis generation: classify residual → process type; query literature + A2A peers; hypothesis registry
- [ ] ML learners: FNO/DeepONet, symbolic regression (PySR), hybrid physics+ML; physics-informed loss; train/val/test split by region
- [ ] Validation gate: skill improvement, conservation (100-yr), stability when coupled, generalization, sensitivity
- [ ] Auto-manifest generation: infer I/O, scale envelope, regime tags, cost/skill model; tag `origin: discovered`
- [ ] Deposit into Process Knowledgebase with full manifest + provenance; immediately available for neural inference and selection
- [ ] Lifecycle: `candidate → provisional → validated → production`; periodic re-validation, retraining, retirement
- [ ] Discovery provenance: residual → hypothesis → training → validation → deployment chain

### 5.17 Geoengineering Strategy Agent
- [ ] Control target registration (setpoint, tolerance, measurement source)
- [ ] Forward simulation pipeline (50–500 yr through coupled ESM)
- [ ] Multi-objective evaluation: cooling efficacy, side-effect penalties, economic cost, termination shock risk
- [ ] Strategy optimization: MPC, RL (PPO/SAC), portfolio optimization
- [ ] Termination shock: cessation simulation, rebound quantification, minimum safe ramp-down
- [ ] Robustness: ECS range, SSP range, technology failure, tipping points
- [ ] Pareto frontier: skill vs. cost vs. side effects vs. termination risk
- [ ] Adaptive control laws (dynamic controllers, not static prescriptions)

### 5.18 Planetary Defense Agent
- [ ] NEO ingest: CNEOS Sentry, Scout, Horizons, MPC, USAF SSN, USSF 18th SDS, ESA NEOCC, survey telescopes
- [ ] Threat assessment: local catalog ranked by Palermo scale; configurable threshold; probability evolution; reports
- [ ] Impact cascade: atmospheric injection, thermal pulse → firestorms, ocean impact → tsunami/acidification, nuclear winter → food web collapse, long-term recovery
- [ ] Deflection: kinetic impactor, gravity tractor, ion beam, nuclear standoff; campaign optimization
- [ ] Mass extinction calibration: K-Pg, P-T, Late Devonian, End-Triassic; compare modeled loss/recovery vs. PBDB
- [ ] Observation campaign recommendations for orbit uncertainty reduction

### 5.19 Trophic Dynamics Agent
- [ ] Food web assembly from diet matrices, stable isotopes, gut content; guild assignment; metabolic theory
- [ ] Calibration: energy flow, biomass pyramids, functional response parameters
- [ ] Coupling: trophic → ecology (herbivory, dispersal), → biogeochem (nutrient cycling), ↔ ocean, ↔ evolution, → human systems
- [ ] Trophic cascade detection

### 5.20 Evolution Agent
- [ ] Trait evolution: distributions (mean, variance, heritability) per population; selection, drift, gene flow
- [ ] Speciation/extinction: reproductive isolation, fitness collapse, adaptive radiation, background extinction rate
- [ ] Validation: diversification rates vs. TimeTree, trait trajectories vs. paleo record, post-extinction recovery timescales
- [ ] Coupling: evolution → ecology, trophic, disturbance, climate, biogeochem
...
- [ ] **Data Discovery** — STAC/registry search; best sources per region/resolution/license
- [ ] **Data Acquisition** — Download/stream with rate limiting, retries, checksums, content-addressed cache
- [ ] **Preprocessing** — Reproject, resample, tile, cloud mask → Zarr/COG
- [ ] **QA/QC** — Completeness, outliers, temporal gaps, uncertainty, fallback triggering

### 6.4 Data Governance
- [ ] Allowlisted providers, credential vault, rate limiting, checksum + content hashing
- [ ] License enforcement, deterministic processing (versioned recipes)

### 6.5 Initial Data Products
- [ ] DEM (Copernicus/SRTM), Sentinel-2 L2A, VIIRS active fire (NRT), precipitation forcing, LAI/NDVI
...
- [ ] Structured enumeration (vary one family's rung); factorial experiments (interaction effects)
- [ ] Budget-aware scheduling; configuration space pruning via consistency checker/posterior

### 7.2 Emulator-Accelerated Screening
- [ ] Train surrogates from completed runs; pre-screen unpromising configurations
- [ ] Track emulator fidelity; retrain as archive grows; uncertainty-aware elimination

### 7.3 Continuous Calibration
- [ ] Hierarchical Bayesian: global → region → regime priors
- [ ] Online parameter updating; identifiability diagnostics

### 7.4 Knowledgebase Feedback Loop
- [ ] Update skill models (replace expert priors with posteriors)
- [ ] Refine cost models with actual walltime/memory
- [ ] Tighten/relax compatibility constraints from observed interactions
- [ ] Shift default rung preferences per regime
- [ ] Discover new regime tags, detect regime boundaries and drift
- [ ] Sharpen parameter priors hierarchically; auto-update manifests

### 7.5 Experiment Orchestrator
- [ ] Consume experiment queue from Active Learning; schedule across compute
- [ ] Lifecycle: compile → run → score → store → update posteriors
- [ ] Budget limits and diminishing-returns stopping criteria; full provenance

### 7.6 Autonomous Data Discovery Loop
- [ ] Periodic gap analysis; integrate Data Scout with experiment orchestrator
- [ ] New product → re-score affected representations → update active learning priorities
- [ ] Autonomous regime discovery: cluster skill records, detect non-default outperformance, update ontology
...
- [ ] Mutual TLS + bearer tokens; A2A client for remote task dispatch

### 8.2 Federated Model Assembly
- [ ] Capability-based delegation to best-qualified remote instance
- [ ] IR fragment exchange; cross-boundary conservation validation
- [ ] Remote execution (boundary conditions via streaming) and colocated modes

### 8.3 Cross-Instance Skill Sharing
- [ ] Anonymized skill record format (config hash + metrics only)
- [ ] Export/import with provenance; trust-weighted Bayesian incorporation
- [ ] Differential privacy safeguards; community posterior dashboard

### 8.4 A2A-Enabled Data Discovery
- [ ] `discover_data`, `share_skill`, `propose_rung` task types
- [ ] A2A-mediated calibration (remote scoring against unique observations)
...
- [ ] HS0 one-way coupling (exogenous scenarios)
- [ ] HS1 bidirectional via MSD Coupling Agent (climate impacts ↔ demand/extraction)
- [ ] HS2 agent-based infrastructure: power grid, water systems, cascading failure — *(stretch)*

### 9.2 Water–Energy–Land Nexus
- [ ] Coupled water-energy accounting; land-use change feedbacks; resource competition resolution

### 9.3 Dynamical Digital Testbeds
- [ ] Testbed compiler: region + stressors → coupled model; evaluation against historical outcomes
- [ ] DOE templates: coastal, western U.S. (fire+water+energy), Arctic

### 9.4 Scenario Space Exploration
- [ ] Ensemble generation (SSP × RCP × technology × policy); AI-driven tipping point discovery
- [ ] Scenario comparison dashboards; uncertainty propagation (climate → impact → decision)
...
- [ ] Pluggable diagnostic packages (manifest + scoring + visualization)
- [ ] CMIP-aligned output formatting (CMOR); automated campaigns after multi-component runs
- [ ] Diagnostic scoring → Skill Score Store integration

### 10.2 RGMA Thrust Diagnostics
- [ ] Cloud & Aerosol: cloud fraction, AOD, CRE, SW cloud feedback
- [ ] Biogeochemical: global C budget, CO₂ flux, N₂O/CH₄, nutrient limitation
- [ ] High-Latitude: permafrost, active layer, sea-ice, ice-sheet mass balance, Arctic amplification
- [ ] Variability: ENSO, AMO, PDO, NAO indices, subseasonal-to-decadal
- [ ] Extreme Events: GEV return periods, compound frequency, attribution
- [ ] Water Cycle: P-ET-R closure, streamflow, soil moisture, groundwater, SWE

### 10.3 Model Hierarchy Analysis
- [ ] Representation ladders as model hierarchy; complexity→skill tradeoff; process attribution
- [ ] Hierarchy-guided uncertainty decomposition (structural vs. parametric vs. data)
...
- [ ] Multi-metric residuals from best configuration; structured bias detection (Moran's I, spectral analysis, regime-conditional)
- [ ] Attribution via partial correlation + causal inference (Granger, transfer entropy)
- [ ] Persistence test (across calibrations, rung swaps, ensemble members, independent products)
- [ ] Severity scoring: magnitude × extent × persistence × relevance

### 11.2 Hypothesis Generation
- [ ] Classify residual → process type: diurnal→energy balance, spatial gradient→lateral transport, post-event→recovery coupling, seasonal phase→phenological feedback
- [ ] Literature + A2A query for known matching processes; hypothesis registry with status tracking

### 11.3 ML Process Learning Pipeline
- [ ] Training data extraction: residual–input pairing, QA/QC filters, region-based cross-validation
- [ ] Neural operators (FNO, DeepONet) + physics-informed loss
- [ ] Symbolic regression (PySR) with dimensional analysis + complexity–accuracy Pareto
- [ ] Hybrid physics+ML (conservation by construction; graceful degradation)

### 11.4 Validation & Knowledgebase Deposit
- [ ] Multi-criteria gate: skill improvement, conservation (100-yr), stability, generalization, sensitivity
- [ ] Auto-manifest with provenance: I/O, scale envelope, regime tags, cost, skill, `origin: discovered`
- [ ] Deposit into Process Knowledgebase with full manifest + provenance; immediately available for neural inference
- [ ] Promotion: `candidate → provisional → validated → production`
- [ ] Lifecycle: periodic re-validation, retraining on expanded data, retirement

### 11.5 Integration With Core Loop
- [ ] Residual analysis on every benchmarking cycle; errors feed neural inference engine
- [ ] Connect to Active Learning, Hypothesis Engine, A2A (`propose_rung`), Process Knowledgebase
- [ ] Discovery budget per cycle; discovery metrics dashboard
...
- [ ] Control targets (setpoint + tolerance + measurement source); error computation
- [ ] Actuator interface per intervention type (control variables)
- [ ] Control loop: observe → error → predict → intervene → simulate → verify → update
- [ ] Integration with autonomous optimization loop

### 12.2 Strategy Discovery & Optimization
- [ ] Intervention schedule representation (time-varying control law, 50–500 yr horizon)
- [ ] MPC: rolling-horizon with ESM forward model; constraint satisfaction; emulator screening
- [ ] RL: PPO/SAC with domain randomization (ECS, SSP uncertainty)
- [ ] Portfolio optimization: synergies (SAI+DAC), antagonisms (iron fert+OAE); budget-constrained allocation

### 12.3 Termination Shock & Stability
- [ ] Cessation simulation at multiple points; rebound rate, overshoot, tipping proximity
- [ ] Minimum safe ramp-down discovery
- [ ] Robustness: ECS 2–5°C, SSP1–5, actuator failure, tipping points (AMOC, WAIS, Amazon, permafrost)
- [ ] Long-term stability verification (500+ yr)

### 12.4 Governance & Safety
- [ ] Side-effect constraint checking (ozone, precipitation, pH, ecosystem impact)
- [ ] Distributional equity analysis (regional winners/losers)
- [ ] Reversibility classification per intervention; strategy provenance
- [ ] Pareto frontier dashboard: cooling vs. cost vs. side effects vs. termination risk
...
- [ ] CNEOS Sentry, Scout, Horizons, MPC, USAF SSN, USSF 18th SDS, ESA NEOCC, survey feeds
- [ ] Automated threat ranking (Palermo scale); alerts on catalog changes

### 13.2 Impact Cascade Modeling
- [ ] Parametric impact model: size/velocity/angle → energy partition
- [ ] Atmospheric injection → AOD → radiative forcing (→ Atmosphere A1/A2)
- [ ] Thermal pulse → firestorm probability (→ Fire F2/F3)
- [ ] Ocean impact → tsunami + acidification + anoxia (→ Ocean O1/O2)
- [ ] Nuclear winter → photosynthesis collapse → trophic cascade (→ TD0-TD2)
- [ ] Long-term recovery → ecosystem rebuilding + adaptive radiation (→ Ecology + Evolution)

### 13.3 Mass Extinction Calibration
- [ ] K-Pg: 10 km impactor; validate impact winter, temperature drop, acid rain, foram extinction, δ¹³C excursion
- [ ] P-T: Siberian Traps; validate warming, anoxia extent, >90% marine loss, 5 Myr delayed recovery
- [ ] Late Devonian, End-Triassic scenarios
- [ ] Compare modeled loss/recovery vs. PBDB stratigraphic range data

### 13.4 Deflection Strategy Assessment
- [ ] Kinetic impactor (DART-class), gravity tractor, ion beam, nuclear standoff
- [ ] Campaign optimization: NEO orbit + lead time → optimal strategy + launch window
- [ ] Post-deflection orbit verification
...
- [ ] Monitor execution metrics per process/region; trigger embedding/upgrades and hysteresis-based downgrades; information-loss accounting

### 14.2 Data & Calibration Agent
- [ ] Datasets per compiled model; parameter priors per rung; identifiability warnings; calibration targets

### 14.3 Streaming / NRT
- [ ] Event bus: `DataUpdateEvent(variable, tiles, time_range, quality_delta)`
- [ ] Active fire → ignition trigger; precip radar → hydro subcycling; burn severity → disturbance application
- [ ] Tile-first processing pipeline
...
- [ ] Schema: `AgentEvent { agent_id, event_type, timestamp, payload, cycle_id, correlation_id }`
- [ ] Types: `task_started/completed/failed`, `skill_recorded`, `data_discovered/ingested`, `process_learned`, `rung_swapped`, `pareto_updated`, `regime_detected`, `a2a_exchange`, `objective_changed`
- [ ] Async bus (Rust channels → append-only log); SQLite (dev) / PostgreSQL (prod)
- [ ] Retention policy (hot 30d, warm 1yr, cold archive); correlation ID propagation

### 15.2 Dashboard Backend
- [ ] REST + WebSocket API: `/events/stream`, `/events/query`, `/skill/timeseries`, `/pareto/frontier`, `/agents/status`, `/data/ingestions`, `/discovery/log`, `/regimes/map`
- [ ] Auth (API key / OAuth); rate limiting + pagination

### 15.3 Dashboard Frontend (Next.js)
- [ ] App Router + React Server Components
- [ ] Views: Agent Workflows (live DAG), Optimization (Pareto animation), Skill Evolution (sparklines), Data Ingestion (feed), Process Discovery (residuals/hypotheses), Provenance Audit (dependency graph), Regime Map (Mapbox/Deck.gl), Federation Status (A2A peers)
- [ ] WebSocket subscriptions; responsive layout

### 15.4 Optional Steering Panel
- [ ] Adjust objectives, weights, budgets, allowlists, process inclusion, domain extent
- [ ] Changes take effect next cycle; logged as `objective_changed` events
...
- [ ] 100–250 m grid, hourly forcing; R0/R1 + H0/H1 + E0 + B0/B1; validate soil moisture, ET, biomass, streamflow

### 16.2 Event Embedding
- [ ] F1 at 30 m for ignitions; disturbance → canopy loss, fuel changes, hydrophobicity; H1/H2 in burned watersheds during storms
- [ ] Show: post-fire runoff peaks, multi-year recovery via succession

### 16.3 Dynamic Rung Switching
- [ ] Auto rung selection (budget + error + triggers); coarse-only vs. embedded-events comparison

### 16.4 Knowledgebase-Driven Assembly
- [ ] Expert-prior KB → neural inference proposals → combinatorial experiments → posterior convergence → deposit learned preferences into KB → recompile → transfer test

### 16.5 Continuous Learning
- [ ] Multi-year hindcast + streaming ingest; show skill improvement, ontology divergence from priors, Active Learning queries

### 16.6 Autonomous Data Discovery
- [ ] Start with gauges only; Data Scout discovers SMAP + MODIS via STAC; re-score all hydrology + ecology; regime detection

### 16.7 Fully Autonomous Operation
- [ ] Input: domain + budget + objectives; system discovers data, assembles, benchmarks, converges, reports — no human intervention

### 16.8 A2A Federated Assembly
- [ ] Two instances (land/fire + ocean/atmos); IR fragment exchange; cross-boundary validation; coupled run; show federated ≥ monolithic skill
...
- [ ] Erosion/sediment transport module family
- [ ] ML emulator backends for fast surrogate execution
- [ ] Arbitrary mesh support + general remapping
- [ ] HPC cluster execution backend
- [ ] RDF/OWL semantic web ontology export
